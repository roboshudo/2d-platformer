...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // 캐릭터 방향 처리.
        xDrection = Input.GetAxis("Horizontal");
        if (Mathf.Abs(xDrection) > 0)
        {
            if (xDrection < 0)
            {
...
public class ClearPlayerPrefs : MonoBehaviour
{
    void Awake()
    {
        ...
        Debug.Log("PlayerPrefs 데이터가 모두 초기화되었습니다.");
    }
}
}
